...
The project runs in demo mode by default, so the UI, movement, login button, and shop can be tested without Pi or Supabase environment variables.

To enable production integrations, set:

```env
NEXT_PUBLIC_DEMO_MODE=false
```

and configure the remaining variables from `.env.example`.
